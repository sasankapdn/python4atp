...
- Next locate the **SSH_PUBLIC_KEY** variable and paste the public ssh key you created earlier in the given field. **It must be in text format.**
...
- Click **select files**, then select the **expdp_alpha.dmp** for import into the database in the next steps.  Click **Open**, then **Upload Objects**.
...
begin
    h1 := dbms_datapump.open (operation => 'IMPORT', job_mode => 'SCHEMA', job_name => 'IMP_ALPHA', version => 'COMPATIBLE'); 
    tryGetStatus := 1;
